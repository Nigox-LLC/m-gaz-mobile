...
import '../../models/technological-measuring/teach_measure_detail/teach_measure_detail.dart';
import '../../models/technological-measuring/technological_measuring.dart';
import '../base/base_api.dart';

class TechMeasureApi {
  final ApiBase _base;

  const TechMeasureApi(this._base);

  Future<PaginatedResponse<TechnologicalMeasuringDocument>> getDocuments({
    int limit = 20,
    int offset = 0,
  }) async {
    try {
      ...
      if (response.statusCode == 200) {
        return PaginatedResponse<TechnologicalMeasuringDocument>.fromJson(
          response.data,
          TechnologicalMeasuringDocument.fromJson,
        );
      } else {
  ...
  Future<TeachMeasureDetail> getDocumentById(int id) async {
    try {
      debugPrint(
      ...
      if (response.statusCode == 200) {
        return TeachMeasureDetail.fromJson(response.data);
      } else {
        throw Exception('Xatolik yuz berdi: ${response.statusCode}');
      }
    } on DioException catch (e) {
